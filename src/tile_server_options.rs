...
fn opt_to_ptr(value: Option<&CStr>) -> *const c_char {
...
impl TileServerOptions {
    ...
    unsafe fn to_opt(&self, value: *const c_char) -> Option<&CStr> {
        unsafe { value.as_ref().map(|v| CStr::from_ptr(v)) }
    }
    ...
    pub fn with_tile_template(
        ...
        unsafe {
            // Slightly inemaplibre_sufficient because CString allocation is than copied to std::string
